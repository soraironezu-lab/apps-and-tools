...
このZIPに含まれるアプリには、改変検出のためのアドホック署名を行っています。ただし、この署名はAppleに登録された開発者の身元を証明するものではありません。Developer ID署名・Appleの公証は行っていません。公式Releaseから入手した場合だけ利用してください。初回起動時にmacOSの警告が表示される場合があります。

最初の警告では「ゴミ箱に入れる」ではなく「完了」を選びます。「システム設定 → プライバシーとセキュリティ」でChatSwitchの許可操作を選び、ChatSwitchをもう一度開いて、確認ダイアログの「このまま開く」を選びます。求められた場合は、管理者のユーザ名とパスワードで許可してください。警告内容を理解できない場合は起動しないでください。
